...
#            train_x = train_x.reshape(len(train_x), n_feats, n_frames,1)
#            crosseval_x = crosseval_x.reshape(len(crosseval_x), n_feats, n_frames,1)  
#            
#            temp_train_x = numpy.concatenate((train_x, crosseval_x), axis=0)
#            temp_train_y = numpy.concatenate((train_y, crosseval_y), axis=0)
#            
#            numpy.random.seed(1)
#            idx = numpy.random.permutation(temp_train_x.shape[0])
#            shuffled_temp_train_x = temp_train_x[idx,:,:,:]
#            shuffled_temp_train_y = temp_train_y[idx,:]
            ...
            model = build_model()
#            model = VGG_16()
#            grid_search(model,shuffled_temp_train_x,shuffled_temp_train_y)
...
#            model.fit(train_x, train_y, validation_data=(valid_x, valid_y), callbacks=[earlystop], batch_size=32, epochs=1)
            model.fit(train_x, train_y, validation_data= None, callbacks=None, batch_size=32, epochs=25)
...
def build_model():
    ...
    model.add(Convolution2D(32, f_size, strides=f_size, kernel_initializer='normal', padding='same', input_shape=(bands, frames, num_channels)))
...
def do_system_testing(dataset, result_path, feature_path, model_path, feature_params,
    ...
    for fold in dataset.folds(mode=dataset_evaluation_mode):
        ...
        if not os.path.isfile(current_result_file) or overwrite:
            ...
            test_x = test_x.reshape(len(test_x), n_feats, n_frames,1)
