...
class Dataset(object):
    ...
    def __init__(self, data_path='data', name='dataset'):
        ...
        # CrossEvaluating meta data for folds
        self.evaluation_data_crosseval = {}
                    ...
                    for f in l:
                        ...
                        if file_extension[1:] in self.audio_extensions:
                            if os.path.abspath(os.path.join(path, f)) not in self.files:
                                print(os.path.abspath(os.path.join(path, f)))
    ...
    def crosseval(self, fold=0):
        """List of cross evaluating items.
        ...
        if fold not in self.evaluation_data_crosseval:
            self.evaluation_data_crosseval[fold] = []
            if fold > 0:
                with open(os.path.join(self.evaluation_setup_path, 'fold' + str(fold) + '_evaluate.txt'), 'rt') as f:
                    for row in csv.reader(f, delimiter='\t'):
                        if len(row) == 2:
                            # Scene meta
                            self.evaluation_data_crosseval[fold].append({
                                'file': self.relative_to_absolute_path(row[0]),
                                'scene_label': row[1]
                            })
                        elif len(row) == 4:
                ...
                self.evaluation_data_crosseval[0] = data

        return self.evaluation_data_crosseval[fold]
...
class TUTAcousticScenes_2017_EvaluationSet(Dataset):
    ...
    def __init__(self, data_path='data'):
        Dataset.__init__(self, data_path=data_path, name='TUT-acoustic-scenes-2017-evaluation')
